  ...
  return (
    <div className="space-y-6 w-full">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <button disabled aria-description="Chức năng chưa khả dụng trong bản dùng thử" className="px-4 py-2.5 rounded-xl bg-surface-inset border border-border hover:border-purple-400 text-slate-200 font-bold text-xs flex items-center gap-2 transition shadow-lg shrink-0">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="p-5 rounded-2xl bg-surface border border-border space-y-2">
          <p className="text-sm text-text-muted">Tổng Doanh Số GMV Hệ Thống</p>
        ...
        <div className="p-5 rounded-2xl bg-surface border border-border space-y-2">
          <p className="text-sm text-text-muted">Lợi Nhuận Chiết Khấu Sàn (5%)</p>
          <p className="text-2xl font-black text-[#39FF14]">{formatPrice(6420000)}đ</p>
          <span className="text-xs text-text-muted">Thu phí sàn duy trì server</span>
        </div>

        <div className="p-5 rounded-2xl bg-surface border border-border space-y-2">
          <p className="text-sm text-text-muted">Số Dư Ví PrintHub Sinh Viên</p>
          <p className="text-2xl font-black text-cyan-400">{formatPrice(45200000)}đ</p>
          <span className="text-xs text-cyan-400 font-bold">142 tài khoản có dư</span>
        </div>

        <div className="p-5 rounded-2xl bg-surface border border-border space-y-2">
          <p className="text-sm text-text-muted">Thanh Toán Cho Các Xưởng In</p>
          <p className="text-2xl font-black text-purple-400">{formatPrice(121980000)}đ</p>
          <span className="text-xs text-text-muted">Đã chi trả 95% giá trị</span>
        </div>
      </div>
          ...
          {[
            ...
            <div key={t.id} className="py-3 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-text-muted text-sm">{t.date}</p>
              </div>
