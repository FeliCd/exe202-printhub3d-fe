...
const mockSystemOrders: SystemOrder[] = [
  {
    ...
    total: 100000,
    assignedFactory: 'Cụm Máy In FDM 3D ĐHQG',
    status: 'PRINTING',
  ...
  },
  {
    ...
    total: 165000,
    assignedFactory: 'Cụm Máy In SPKT',
    status: 'COMPLETED',
    paymentMethod: 'Cổng PayOS',
  },
  {
    ...
    total: 95000,
    assignedFactory: 'Chưa phân bổ máy in',
    status: 'PENDING',
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <div className="space-y-4">
        {filteredOrders.length === 0 && <p role="status" className="p-6 text-slate-300">Không có đơn hàng ở trạng thái này.</p>}
        {filteredOrders.map(o => (
          <div key={o.id} className="p-5 rounded-2xl bg-surface border border-border text-xs space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-border pb-3">
              <div>
                ...
                <p className="text-text-muted text-sm">Ngày đặt: {o.date} • {o.paymentMethod}</p>
              </div>
            ...
            <div className="p-3 bg-surface-inset rounded-xl border border-border space-y-1">
              <span className="text-text-muted block">Sản phẩm:</span>
              <strong className="text-white">{o.itemsSummary}</strong>
              <p className="text-sm text-cyan-300 mt-1">Xưởng in phân bổ: <strong className="text-white">{o.assignedFactory}</strong></p>
            </div>

            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-1 border-t border-border">
              <span className="text-text-muted">Tổng giá trị đơn: <strong className="text-base font-black text-[#39FF14] font-mono">{formatPrice(o.total)}đ</strong></span>
