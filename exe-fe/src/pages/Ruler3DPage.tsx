  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 p-6 rounded-3xl bg-[#111215] border border-[#272930] flex flex-col justify-between min-h-[380px] relative overflow-hidden group">
          ...
          <div className="relative z-10 flex items-center justify-between">
            ...
            <div className="flex items-center gap-1.5 bg-[#18191d] border border-[#272930] rounded-xl p-1 text-xs">
              <button
                onClick={() => setUnit('mm')}
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  unit === 'mm' ? 'bg-[#22c55e] text-slate-950' : 'text-slate-400 hover:text-white'
                }`}
              >
                mm
              </button>
              <button
                onClick={() => setUnit('cm')}
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  unit === 'cm' ? 'bg-[#22c55e] text-slate-950' : 'text-slate-400 hover:text-white'
                }`}
              >
                cm
              </button>
              <button
                ...
                className={`px-2.5 py-1 rounded-lg font-bold transition ${
                  unit === 'inch' ? 'bg-[#22c55e] text-slate-950' : 'text-slate-400 hover:text-white'
                }`}
              >
                inch
              </button>
          ...
          <div className="relative z-10 my-8 flex items-center justify-center">
            ...
            {modelType === 'caliper' && (
              <div className="w-72 h-20 bg-zinc-800 rounded-xl border-2 border-zinc-600 p-3 flex items-center justify-between text-slate-200 font-mono text-xs">
                ...
                <div className="w-12 h-12 bg-[#22c55e] text-slate-950 font-bold rounded-lg flex items-center justify-center text-[10px]">
                  Cữ đo
                </div>
              </div>
            )}
          ...
          <div className="relative z-10 flex flex-wrap items-center justify-between text-xs text-[#94a3b8] pt-3 border-t border-[#272930]">
            ...
            <span className="text-[11px] text-slate-400">Thể tích lưới mesh: {((dims.x * dims.y * dims.z) / 1000).toFixed(2)} cm³</span>
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="p-5 rounded-2xl bg-[#18191d] border border-[#272930] space-y-4">
            ...
            <div className="space-y-2 text-xs">
              <button
                ...
                className={`w-full p-3 rounded-xl border font-bold text-left flex items-center justify-between transition ${
                  modelType === 'ruler20' ? 'border-[#22c55e] bg-[#22c55e]/15 text-[#22c55e]' : 'border-[#272930] bg-[#111215] text-slate-300'
                }`}
              >
              ...
              <button
                ...
                className={`w-full p-3 rounded-xl border font-bold text-left flex items-center justify-between transition ${
                  modelType === 'caliper' ? 'border-[#22c55e] bg-[#22c55e]/15 text-[#22c55e]' : 'border-[#272930] bg-[#111215] text-slate-300'
                }`}
              >
              ...
              <button
                ...
                className={`w-full p-3 rounded-xl border font-bold text-left flex items-center justify-between transition ${
                  modelType === 'cube' ? 'border-[#22c55e] bg-[#22c55e]/15 text-[#22c55e]' : 'border-[#272930] bg-[#111215] text-slate-300'
                }`}
              >
            ...
            {modelType === 'caliper' && (
              <div className="pt-2 border-t border-[#272930] space-y-2 text-xs">
                <label className="font-bold text-slate-300">Thao tác trượt cữ đo thước kẹp:</label>
                <input
                  type="range"
                  min={0}
                  max={50}
                ...
                />
                <p className="text-[11px] text-[#22c55e] font-bold font-mono">Độ chia du xích: {caliperValue.toFixed(2)} mm</p>
              </div>
            )}
          </div>
        </div>
