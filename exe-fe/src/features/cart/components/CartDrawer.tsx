...
export default function CartDrawer({
  ...
  return (
    <Modal open={isOpen} onClose={onClose} label="Giß╗Å h├áng cß╗ºa bß║ín" drawer>
    <div className="w-full h-full bg-surface-inset border-l border-border flex flex-col">
      ...
      <div className="p-4 border-b border-border flex items-center justify-between bg-surface">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-bold text-white text-base">Giß╗Å H├áng Cß╗ºa Bß║ín</h3>
          <span className="text-xs bg-primary/20 text-[#22c55e] px-2 py-0.5 rounded-full font-bold">{totalItems} m├│n</span>
        </div>
        <button aria-label="─É├│ng giß╗Å h├áng" className="text-slate-400 hover:text-white p-1 rounded-lg hover:bg-[#272930] transition" onClick={onClose}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        <div className="space-y-3">
          {items.length === 0 && <p role="status" className="py-6 text-sm text-slate-300">Giß╗Å h├áng ─æang trß╗æng. Th├¬m sß║ún phß║⌐m ─æß╗â tiß║┐p tß╗Ñc.</p>}
          {items.map((item) => (
        ...
        {/* Voucher */}
        <div className="pt-2">
          <label htmlFor="cart-coupon" className="block text-xs font-bold text-slate-300 uppercase mb-1.5">M├ú Giß║úm Gi├í Sinh Vi├¬n</label>
          <div className="flex gap-2">
            <input
              ...
              placeholder="Nhß║¡p m├ú (Vd: SINHVIEN2024)"
              type="text"
              value={localCoupon}
          ...
          {couponApplied && (
            <p className="text-sm text-emerald-400 mt-1 flex items-center gap-1">
              ...
              ─É├ú giß║úm {formatPrice(discount)}─æ tß╗½ Voucher T├ón Sinh Vi├¬n!
            </p>
          )}
        </div>
        ...
        <div className="p-3.5 rounded-xl bg-surface border border-border space-y-2.5">
          ...
          <div className="p-2.5 rounded-lg bg-surface-inset border border-border text-xs space-y-1">
            <div className="flex items-center justify-between font-bold text-slate-200">
              <span>{shippingAddress.recipientName} ΓÇó {shippingAddress.phone}</span>
              <span className="text-xs bg-primary/20 text-[#22c55e] px-1.5 py-0.5 rounded font-bold uppercase">─É├ú chß╗ìn</span>
            </div>
        ...
        <div className="text-xs text-slate-400 bg-emerald-950/30 border border-emerald-900/40 p-2.5 rounded-lg flex items-center gap-2">
          ...
          <span>Cam kß║┐t: 1 ─æß╗òi 1 miß╗àn ph├¡ nß║┐u th╞░ß╗¢c g├úy hoß║╖c sai vß║ích ─æo trong 1 hß╗ìc kß╗│!</span>
        </div>
      </div>
      ...
      <div className="p-4 border-t border-border bg-surface space-y-3">
        <div className="space-y-1.5 text-xs text-text-muted">
          <div className="flex justify-between">
            <span>Tß║ím t├¡nh ({totalItems} th╞░ß╗¢c):</span>
            <span className="text-slate-200 font-medium">{formatPrice(subtotal)}─æ</span>
          </div>
          {couponApplied && (
            <div className="flex justify-between">
              <span>M├ú giß║úm gi├í ({couponCode}):</span>
              <span className="text-emerald-400 font-medium">-{formatPrice(discount)}─æ</span>
            </div>
          )}
          <div className="flex justify-between">
            <span>Ph├¡ vß║¡n chuyß╗ân KTX / Nß╗Öi th├ánh:</span>
            <span className="text-slate-200 font-medium">{formatPrice(shippingFee)}─æ</span>
          </div>
          <div className="flex justify-between text-sm font-black text-white pt-2 border-t border-border">
            <span>Tß╗òng thanh to├ín:</span>
            <span className="text-[#22c55e] text-base">{formatPrice(total)}─æ</span>
          </div>
        </div>
